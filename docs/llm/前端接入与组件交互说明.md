...
P1 后端自动路由已经完成。前端不再根据文字猜测意图，可统一发送：
...
P1 推荐快照已经上线。首次推荐后改为传同一 `conversation_id`，需要指定路线时附加 `route_id`，不再由前端回传完整路线。
...
点击换一条
→ question="换一条"
→ 携带当前 conversation_id
→ POST /api/v1/ai/travel
→ 后端从推荐快照选择下一条
→ related_routes 更新 AiRouteCard
```

该调用不会重新执行推荐服务，只会轮换当前会话快照；用户改变起终点或明确要求重新推荐时才生成新候选集。
