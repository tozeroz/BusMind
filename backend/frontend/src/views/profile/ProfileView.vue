  ...
  <section class="page-grid">
    <div class="panel">
      ...
      <form class="admin-form-row" @submit.prevent="saveNickname">
        <input v-model="nickname" maxlength="32" placeholder="修改昵称" />
...
const loadProfile = async () => {
  ...
  try {
    ...
    message.value = error?.response?.status === 401 ? '登录已失效，请重新登录' : getApiErrorMessage(error, '个人数据加载失败')
  } finally {
    loading.value = false
  }
}
...
const saveNickname = async () => {
  try {
    ...
    message.value = getApiErrorMessage(error, '昵称更新失败')
  }
}
...
const addFavorite = async () => {
  ...
  try {
    ...
    message.value = getApiErrorMessage(error, '添加收藏失败')
  }
}
