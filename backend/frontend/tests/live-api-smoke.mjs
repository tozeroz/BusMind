...
if (stations.length >= 2) {
  try {
    ...
    if (
      String(error?.message || error).includes(
        'HTTP 404 no route recommendation found'
      )
    ) {
      skip(
        'route recommendation',
        `站点 ${stations[0].station_id} 到 ${stations[1].station_id} 暂无可推荐路线`
      )
    } else {
      throw error
    }
  ...
  skip('route recommendation', '数据库不足两个站点')
}
...
async function callOrSkip(name, path, options = {}, skippableStatuses = [404]) {
  const response = await fetch(`${base}${path}`, {
    headers: {
  ...
  if (!response.ok && skippableStatuses.includes(response.status)) {
    skip(
      name,
      payload?.message ||
  ...
  if (!response.ok) {
    throw new Error(
      `${name}: HTTP ${response.status} ${
        payload?.message || JSON.stringify(payload?.detail)
      }`
    )
  }
