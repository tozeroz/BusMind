...
class MySQLTransitGateway:
    ...
    async def get_latest_eta_status(
        ...
        if latest is not None:
            return EtaStatusData(
                ...
                arrival_time=latest.arrival_time or latest.query_time + timedelta(minutes=float(latest.eta_minutes)),
        ...
        return EtaStatusData(
            ...
            arrival_time=recent.estimated_arrival or recent.query_time + timedelta(minutes=float(recent.eta_minutes)),
    ...
    async def get_distance_to_station_meters(self, vehicle_id: int, target_station_id: int) -> float:
        latest_eta = await self.get_latest_eta_status(vehicle_id=vehicle_id, target_station_id=target_station_id)
    ...
    def _eta_from_cache(self, vehicle_id: int, target_station_id: int, line_id: int | None, station: Any) -> EtaStatusData | None:
    ...
    def _load_from_cache(self, line_id: int, station_id: int, vehicle_id: int | None, station: Any) -> LoadStatusData | None:
        arrival = self._arrival_from_cache(line_id, station)
        if arrival is None:
            return None
    ...
    def _arrival_from_cache(self, line_id: int | None, station: Any) -> Any | None:
        if station is None or not getattr(station, "bus_stop_code", None):
            return None
        bus_stop_code = str(station.bus_stop_code)
        line = self.repository.get_line(line_id) if line_id is not None else None
