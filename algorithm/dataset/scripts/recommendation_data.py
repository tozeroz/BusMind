"""Shared data loaders for recommendation training datasets."""
...
def load_traffic_speed_bands(processed_dir: Path, raw_dir: Path) -> pd.DataFrame:
    ...
    for path in sorted((raw_dir / "traffic_speed_bands").glob("*/*.json")):
        ...
        query_time = (
            pd.to_datetime((payload or {}).get("query_time"), errors="coerce")
...
def station_flow_hourly_lookup(passenger_flow_trend: pd.DataFrame) -> pd.DataFrame:
