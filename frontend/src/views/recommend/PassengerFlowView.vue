...
import { getPassengerFlowPrediction, getPassengerFlowTrend, getLoadPredictionsByLine, getLoadPrediction } from '@/api/history'
...
const loadFlow = async () => {
  ...
  try {
    const [trendResponse, predictionResponse] = await Promise.all([
      getPassengerFlowTrend({ granularity: granularity.value }),
      getPassengerFlowPrediction({})
    ])
    const trend = trendResponse.data || {}
    trendItems.value = trend.items || []
    summary.value = trend.summary || summary.value
    predictionItems.value = Array.isArray(predictionResponse.data) ? predictionResponse.data : predictionResponse.data?.items || []
  } catch (error) {
    trendItems.value = []
